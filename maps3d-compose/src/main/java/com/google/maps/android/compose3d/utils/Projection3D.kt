 ...
 */
data class ScreenCoordinate(val x: Float, val y: Float, val isVisible: Boolean, val depth: Double) {
    ...
    /** Converts the projected position to a Jetpack Compose [Offset]. */
 ...
 * 2D screen viewport pixel space ([ScreenCoordinate] / [Offset] / [PointF]).
 *
...
class Projection3D(
    ...
    /**
     * Projects a 3D world coordinate to a Compose [Offset].
     *
     * @param point The 3D coordinate to project.
     * @return An [Offset] on screen, or null if the point is behind the camera.
     */
    fun toScreenOffset(point: LatLngAltitude): Offset? {
        val coord = toScreenCoordinate(point)
        return if (coord.depth > 0.0) coord.toOffset() else null
    }
     ...
     */
    fun fromScreenLocation(
        screenX: Float,
        screenY: Float,
        targetAltitude: Double = 0.0,
    ): LatLngAltitude? {
