...
- **The Solution:** We swapped to a true WebGL `Marker3DElement`.
  ...
  2. We enabled `extruded: true` and boosted the absolute position strictly to `terrainAltitude + 5m`. 
  This creates a beautiful, volumetric 5-meter red laser "Leader Line" piercing straight down from the camera's focus point directly into the exact terrain surface, perfectly anchoring the viewer.
...
- **The Solution (Part 1 - Spatial Gradients):** At every frame, the engine scans the absolute delta of terrain variance spanning roughly the upcoming **500 meters**. If a high variance (steep climb) is immediately oncoming, it triggers highly aggressive multipliers:
  ...
  3. It violently lifts the camera's `tilt` dynamically—pushing from a scenic pitch (`65` degrees) straight up toward the orthogonal horizon (`85` degrees). This forces the camera to "look up" and preserve perspective, preventing it from staring straight down into the mountain face. 
- **The Solution (Part 2 - Dual-Path Pre-Baking Algorithm):** If the 3D ground geometry *behind* the target is higher than the target (like panning horizontally along a sheer cliff in Peru), simply raising the target altitude isn't enough; the trailing physical lens still smashes into the cliff. We built a permanent algorithmic fix in `app.js`: 
  1. Before the camera starts, we decode the GPS route and calculate a **second algorithmic path**: a strict 1000m trailing drone footprint tracking behind every coordinate using the forward bearing.
  2. We pass *both* paths into a `Promise.all()` batch call to `google.maps.ElevationService` to extract the true 3D spatial ground elevations.
  3. We inject `cameraGroundAltitude` permanently into the active `routeCoordinates` array. 
  4. At 60 FPS, `camera.js` now executes synchronous zero-latency lookups against the true terrain height directly under its physical lens (`cameraTerrainElevation = currentPointInfo.point.cameraGroundAltitude`), forcing the `cameraAltitude` to blindly ride `+250m` above incoming cliff faces instantly.
...
- **The Solution:** We abstracted the camera setter into a pure, stateless spatial function: `drawCameraAtDistance(distance_in_meters)`. The temporal loop simply accumulates elapsed distance over time and calls that function. 
  By exposing the same spatial function to a standard HTML `<input type="range" id="scrubber">`, users can simply grab the slider and scrub forwards/backwards millions of frames at will, and the engine flawlessly redraws the cinematic camera state **even if playback is fully paused.**
